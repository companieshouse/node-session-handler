...
function sessionRequestHandler(config, sessionStore) {
    return (request, response, next) => __awaiter(this, void 0, void 0, function* () {
        ...
        if (sessionCookie) {
            Logger_1.loggerInstance().info(`Session cookie - REQUEST: ${request.url} - COOKIE: ${sessionCookie}`);
            try {
            ...
            Logger_1.loggerInstance().info(`No session cookie - REQUEST: ${request.url}`);
            delete request.session;
        }
        return next();
    });
