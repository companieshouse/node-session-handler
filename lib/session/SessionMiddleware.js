...
const createSessionCookie = (request, config, response, sessionStore) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    Logger_1.loggerInstance().info(`do we have a cookie in request after response.cookie is run? ${JSON.stringify(request.cookies)}`);
    request.cookies = [cookie];
    Logger_1.loggerInstance().info(`do we have a cookie in request after we hard code it in? ${JSON.stringify(request.cookies)}`);
