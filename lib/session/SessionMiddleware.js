...
const sessionRequestHandler = (config, sessionStore) => {
    ...
    return (request, response, next) => __awaiter(void 0, void 0, void 0, function* () {
                    ...
                    if (request.session != null && hash(request.session) !== originalSessionHash) {
                        ...
                        try {
                            ...
                            if (sessionCookie) {
                                ...
                                Logger_1.loggerInstance().info(`creating new session cookie`);
                                cookie = Cookie_1.Cookie.createNew(config.cookieSecret);
                            }
                            yield sessionStore.store(cookie, request.session.data, config.cookieTimeToLiveInSeconds || DEFAULT_COOKIE_TIME_TO_LIVE_IN_SECONDS);
                        }
                        catch (err) {
        ...
        if (sessionCookie) {
            ...
            Logger_1.loggerInstance().info(`generated sessionId: ${sessionId}`);
            const signature = CookieUtils_1.generateSignature(sessionId, config.cookieSecret);
            Logger_1.loggerInstance().info(`generated signature: ${signature}`);
            session.data = { [SessionKey_1.SessionKey.Id]: sessionId + signature };
