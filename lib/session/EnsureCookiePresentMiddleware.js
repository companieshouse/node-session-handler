...
exports.EnsureSessionCookiePresentMiddleware = EnsureSessionCookiePresentMiddleware;
 ...
 */
function EnsureSessionCookiePresentMiddleware(config) {
    const { redirectHeaderName, redirectHeaderValue } = redirectionParametersSupplier(config);
    const hasPreviouslyBeenRedirected = previousRedirectPredicateFactory(redirectHeaderName, redirectHeaderValue);
    return (req, res, next) => {
 ...
 */
const previousRedirectPredicateFactory = (redirectHeaderName, redirectHeaderValue) => (req) => req.get(redirectHeaderName) === redirectHeaderValue;
/**
 ...
 */
const redirectionParametersSupplier = (config) => ({
...
//# sourceMappingURL=EnsureCookiePresentMiddleware.js.map